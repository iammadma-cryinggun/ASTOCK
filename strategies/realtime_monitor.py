...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('D:/daily_stock_analysis/logs/strategy_monitor.log', encoding='utf-8'),
...
# 数据路径
DATA_PATH = r'D:\期货数据\沪铜4小时K线_20260203_130227.csv'
...
def load_position_status():
    """加载当前持仓状态"""
    status_path = Path('D:/daily_stock_analysis/logs/position_status.json')
...
def run_monitoring():
    ...
    logger.info(f"数据加载成功: {df['datetime'].iloc[0]} ~ {df['datetime'].iloc[-1]} ({len(df)} 根)")
